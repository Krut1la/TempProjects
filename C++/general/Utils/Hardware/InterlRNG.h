...
__declspec( align(16) ) static __m128i cur_seed[4];  
 
void static inline srand_sse( unsigned int seed, unsigned int iSeedCell ) 
{ 
   cur_seed[iSeedCell] = _mm_set_epi32( seed, seed+1, seed, seed+1 ); 
} 
 
inline void rand_sse( unsigned int* result, unsigned int iSeedCell ) 
{ 
   ...
   cur_seed_split = _mm_shuffle_epi32( cur_seed[iSeedCell], _MM_SHUFFLE( 2, 3, 0, 1 ) ); 
   cur_seed[iSeedCell] = _mm_mul_epu32( cur_seed[iSeedCell], multiplier ); 
   multiplier = _mm_shuffle_epi32( multiplier, _MM_SHUFFLE( 2, 3, 0, 1 ) ); 
   cur_seed_split = _mm_mul_epu32( cur_seed_split, multiplier ); 
   cur_seed[iSeedCell] = _mm_and_si128( cur_seed[iSeedCell], mod_mask); 
   cur_seed_split = _mm_and_si128( cur_seed_split, mod_mask ); 
   cur_seed_split = _mm_shuffle_epi32( cur_seed_split, _MM_SHUFFLE( 2, 3, 0, 1 ) ); 
   cur_seed[iSeedCell] = _mm_or_si128( cur_seed[iSeedCell], cur_seed_split ); 
   cur_seed[iSeedCell] = _mm_add_epi32( cur_seed[iSeedCell], adder); 
 
#ifdef COMPATABILITY 
 
   // Add the lines below if you wish to reduce your results to 16-bit vals... 
 
   sseresult = _mm_srai_epi32( cur_seed[iSeedCell], 16); 
   ...
   _mm_storeu_si128( (__m128i*) result, cur_seed[iSeedCell]); 
